...
calculateMonths dA dB =
    let
        month1 = monthToIntMaybe(month(dA))
        month2 = monthToIntMaybe(month(dB))
        year1 = year(dA)
        year2 = year(dB)
        monthsInYears = (year2 - year1 - 1) * 12
    in
     ...
     hasMonth date = 
            case month(date) of
                ...
                Nothing -> 
                    False
    in
     if (hasMonth dA) && hasMonth(dB) == True then Just (calculateMonths dA dB) 
     else if (hasMonth dA) || (hasMonth dB) == True then Nothing 
          else Just (calculateYears dA dB)
...
monthToIntMaybe : Maybe Month -> Int
monthToIntMaybe mon =
    case mon of
        Just Jan -> 1
        Just Feb -> 2
...
monthToStringMaybe :  Maybe Month -> String
monthToStringMaybe mon = 
    case mon of
        Just Jan -> 
            "January"
        Just Feb -> 
