...
compare (Interval intA) (Interval intB) =
    let
        ...
        compareEnd (Interval inA) (Interval inB) = 
            ...
            if (existsEnd <| Interval <| inA)  && (existsEnd <| Interval <| inB) == True then Date.compare (getEnd <| Interval <| inA) (getEnd <| Interval <| inB)
            else checkOpenYear (Interval inA) (Interval inB)

        checkOpenYear (Interval inA) (Interval inB) =

           if (existsEnd <| Interval <| inA)  && (existsEnd <| Interval <| inB) == False then EQ
           else
